...
use Imrancse94\Grocery\app\Exceptions\InvalidGroceryToken;
...
class JwtHelper
{
    ...
    public static function validateToken($type,$token)
    {
        ...
        if($payload->token_type != $type){
            throw new InvalidGroceryToken('Invalid token');
        }

        if(self::isTokenExpired($payload)){
            throw new GroceryTokenExpiredException('Token Expired');
        }
